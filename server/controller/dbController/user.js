...
export const signup = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({email: response.rows[0].email}, process.env.SECRET_KEY, { expiresIn: '12h'});
    const data = {
      token,
...
//     const token = jwt.sign({email: foundUser.rows[0].email}, process.env.SECRET_KEY, { expiresIn: '12h'});
//     return responseMsg(res, 200, 'success', token);
//   } catch (error) {
//     return res.status(404).json(error);
//   }
// };